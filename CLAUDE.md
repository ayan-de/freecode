...
| Web-session provider | `specs/2026-08-29-gemini-web-provider.md` — incl. the measurements behind `supportsTools: false`. §10: opt-in text-protocol tool bridge (`tool-bridge.ts`, `experimentalTools`/`FREECODE_GEMINI_WEB_TOOLS=1`) — reply must be `[TOOL_CALLS]` or `FINAL:`, violations get one gated retry before anything streams; default stays no-tools |
