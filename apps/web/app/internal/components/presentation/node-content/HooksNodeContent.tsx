...
export function HooksNodeContent() {
  const [activeHookSafety, setActiveHookSafety] = useState<"safe" | "unsafe">(
    "safe",
  );
  ...
  return (
    <>
      ...
      <p className={styles.description}>
        To protect the user's computer, FreeCode pipes every tool call through a
        strict <strong>Hooks Middleware Pipeline</strong>. Pre-hooks inspect the
        command and can completely block malicious queries.
      </p>
            ...
            {activeHookSafety === "safe" ? (
              <div className={styles.hookConsoleLog}>
                <div className={styles.hookLogLine}>
                  <span className={styles.hookInfo}>[INFO]</span> Intercepting
                  tool request: <code>read_file("src/db.ts")</code>
                </div>
                <div className={styles.hookLogLine}>
                  <span className={styles.hookPre}>[PRE-EXEC]</span> Scanning
                  parameters... Safe action verified.
                </div>
                <div className={styles.hookLogLine}>
                  <span className={styles.hookSuccess}>[STATUS]</span> Executing
                  tool read_file...
                </div>
                <div className={styles.hookLogLine}>
                  <span className={styles.hookPost}>[POST-EXEC]</span> Action
                  logged. Returning output chunk (142 bytes) to AI loop.
                </div>
              </div>
            ) : (
              <div className={styles.hookConsoleLog}>
                <div className={styles.hookLogLine}>
                  <span className={styles.hookInfo}>[INFO]</span> Intercepting
                  tool request: <code>run_bash("rm -rf /")</code>
                </div>
                <div className={styles.hookLogLine}>
                  <span className={styles.hookPre}>[PRE-EXEC]</span> Scanning
                  parameters...{" "}
