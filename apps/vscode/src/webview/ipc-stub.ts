...
export async function sessionStart(config: { projectPath: string; provider?: string }): Promise<SessionInfo> {
  ...
  return withTimeout(promise, 15000, 'sessionStart timed out — CLI may not have started');
}
...
export async function sessionSend(sessionId: string, message: string): Promise<unknown> {
  ...
  return withTimeout(promise, 30000, 'sessionSend timed out — AI may be slow or not responding');
}
