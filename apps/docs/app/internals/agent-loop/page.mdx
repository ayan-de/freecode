...
7. **Heuristic D — repeated reasoning similarity — was never implemented.**
   ...
   heuristics.
8. **A loop-health `warn` still reaches nobody.** `stagnantTurns` now counts
   turns, and `oscillationScore` now decays, so the signal is trustworthy — but
   every `warn` is still written with `logger.debug` (`loop.ts:737`), invisible
   at the default log level and never shown to the model. Nothing acts on a
   detected stuck pattern until it doubles into a `stop`. Closing that is
   `docs/superpowers/specs/2026-08-26-trajectory-redirection.md` Phase 1.
