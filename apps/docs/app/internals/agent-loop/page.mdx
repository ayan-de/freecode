...
1. **The `Stop` hook never fires on a normal finish.** It runs only from
...
2. **`SessionStart` fires once per user message, not once per session.** It sits
   inside `run()` (`loop.ts:593`), which is called for every prompt.
3. **The tree watcher cannot affect the current session's prompt.** `run()` calls
...
4. **Heuristic D — repeated reasoning similarity — was never implemented.**
   ...
   heuristics.
5. **A loop-health `warn` reaches nobody unless redirection is switched on.**
...
6. **`validateParams` in the orchestrator is dead** (`tools/orchestrator.ts:64`)
