...
2. **`getProvider()` constructs a new client on every call**
...
3. **`readConfig()` has no error handling** (`config.ts:39`). A malformed
...
4. **`ProviderDefinition.create(apiKey)` ignores its argument.** All six
   adapters take `_apiKey` and call `getApiKey()` themselves, and `getProvider`
   passes `""`. The parameter is vestigial and misleading.
5. **`summarizeCache`'s hit ratio is wrong and unused.** It computes
...
6. **A total cache failure is the one thing the detector can't see.** The loop's
...
7. **Only `anthropic` counts as a caching provider for the cold warning.**
...
8. **A changing tool set busts the cache with no journal entry.** When an MCP
...
9. **`ExecuteResult.thinking` is dead for every provider.** All six set
...
10. **`ProviderInfo.maxOutputTokens`'s documentation is stale.** Its comment
...
11. **`session/normalize/` is unreachable.** The v4 spec describes a
...
12. **`browser/` has zero importers.** `CLAUDE.md` calls the Playwright path
