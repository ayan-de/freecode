...
```mermaid
graph LR
    A["registry.ts"] -->|"initProviders()"| B["import each adapter"]
    B -->|"side effect"| C["registerProvider(id, { info, create })"]
```
...
```mermaid
graph TD
    A["Message[]"] -->|"convertToCoreMessages"| A2["ModelMessage[]"]
    B["ToolDef[]"] -->|"buildToolsParam"| B2["{ name: { description, inputSchema } }"]
    C["SystemBlock[]"] -->|"buildAnthropicSystemParam / join"| C2["system"]
    A2 --> D["generateText() / streamText()"]
    B2 --> D
    C2 --> D
```
