...
| --- | --- | --- | --- |
| `trajectory` | did the agent reach for the **right tool**? | pure fold over the trace — no files touched | 22 |
...
21/22 cases passed (1 trial each)
...
| `expectParallelTools` | at least one response must emit **this many tool calls at once**. Guards the parallel-batching behaviour: a prompt edit can silently serialize the loop back to one call per turn, and only a model-backed case can see it. Scored off what the response *emitted*, so a batch the permission layer denied still counts. Values below 2 are rejected — every response with a tool call is a "batch" of 1 |
