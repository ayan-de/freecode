...
```
┌─ system ────────────────────────────────────────────── cache: true ─┐
│ 1. session/prompt/system.md      the base prompt                    │
│ 2. "You are powered by <model>"  identity grounding                 │
│ 3. MODE_PROMPTS[agentMode]       plan / build / review / explore     │
│ 4. CLAUDE.md / AGENTS.md         global, then project               │
│ 5. Available Skills              name + description only, sorted    │
│ 6. Memory guidance               constant text                      │
├─ system ───────────────────────────────────────────── cache: false ─┤
│ 7. Compacted session summary     changes only on compaction         │
│ 8. Relevant memories             per turn, retrieved                │
│ 9. Todo list                     per turn, from the todo store      │
│ 10. <system-reminder> blocks     one turn only, then dropped        │
└─────────────────────────────────────────────────────────────────────┘
┌─ messages[0] — a *user* message ────────────────────────────────────┐
│ Project: <name>                                                     │
│ Path: <projectPath>                                                 │
│                                                                     │
│ File tree:                                                          │
│   📁 apps                                                           │
│   📄 package.json          ← top level only, see below              │
│                                                                     │
│ Current Time: 2026-08-23T14:00:00Z    ← rounded to the hour         │
└─────────────────────────────────────────────────────────────────────┘
┌─ messages[1..] ── the actual conversation ──────────────────────────┐
```
...
## Three caches, and how they interact
...
| compiler's `fileTreeCache` | `projectPath:gitHead:ignorePatterns` | 5 min TTL | `PromptCompiler.clearCaches()` |
...
1. **The git HEAD never reaches the model.** It is computed with an `execSync`
   per cache miss, stored on `ProjectContext`, frozen per session, threaded
   through the loop and passed into `compileDynamicContext` — and
   `compileProjectSummary` uses it only as a **cache key**. The rendered section
   is `Project` / `Path` / `File tree` and nothing else (`compiler.ts:115`).
   `CLAUDE.md` and this codebase's own comments describe git HEAD as part of what
   the model is given; it isn't.
2. **The compiler's `fileTreeCache` is keyed on something the value doesn't
   depend on.** The key is `projectPath:gitHead:ignorePatterns`, but the cached
   string is built from `tree`, which changes independently of HEAD (any new
   top-level file). So a second session started within 5 minutes renders the
   *first* session's tree even though the watcher just refreshed it — the one
   benefit the watcher provides is cancelled by the layer above it. The cache
   also saves only a four-line string concatenation.
...
5. **Editing `CLAUDE.md` mid-session busts the prompt cache silently.**
   `compileInstructionsSection` re-reads from disk every turn and feeds the
   `cache: true` block, so an edit changes the first breakpoint and re-bills
   everything after it. Legitimate — but nothing calls `recordInvalidation`, so
   [the cache-miss detector](/internals/providers#cache-observability) reports it
   as an unexplained bust, which is the false positive the journal exists to
   prevent. (Same shape as the MCP tool-set gap.)
6. **The 40,000-character instruction cap truncates mid-file, after joining.**
   Global instructions render first, so a large global `CLAUDE.md` can silently
   push the project's own file out of the prompt. The truncation marker says the
   character count but not *which* file was cut.
7. **Instruction resolution is root-only.** No walk-up for monorepos, no
   ...
   `TODO.md` as extensibility item 4.
