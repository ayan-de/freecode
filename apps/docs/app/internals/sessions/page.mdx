...
1. **The thread store is written but not used** (above). Two systems answer "what
   sessions exist?"; only one of them is consulted.
2. **Delete leaves everything on disk.** A status flag is not erasure, which
   matters if a transcript captured something sensitive.
3. **`session/normalize/` is dead code.** Those provider-response normalizers
...
4. **Session end is not a signal.** There is no "session finished" event, which is
