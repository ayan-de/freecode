...
```
frontend ──▶ {"jsonrpc":"2.0","id":7,"method":"session.send","params":{…}}
                                                                    core
core ──────▶ {"type":"tool_start","toolCallId":"t1","toolName":"read",…}
core ──────▶ {"type":"text_delta","delta":"Looking at "}
core ──────▶ {"type":"done","content":"…"}
core ──────▶ {"jsonrpc":"2.0","id":7,"result":{…}}
```
...
```
session.send ──▶ busy? ──yes──▶ enqueue, resolve immediately { queued: true, id }
                   │                    └─▶ message_queued event
                   no
                   ▼
              run the loop ──▶ hundreds of stream events ──▶ resolve with the result
```
...
```
core ──▶ { type: "question_asked",   requestId, questions }
                                          ▼
frontend ──▶ question.answer { requestId, answers }   (or question.reject)

core ──▶ { type: "permission_asked", requestId, toolName, args, description,
                                     suggestedRule, reason }
                                          ▼
frontend ──▶ permission.answer { requestId, decision, editedRule? }
```
