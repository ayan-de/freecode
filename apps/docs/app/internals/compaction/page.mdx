...
```mermaid
flowchart TD
    A["every turn"] --> B["maybeCompact()"]
    B --> C["shouldCompact(model, contextLimit, measuredTokens)"]
    C --> D["measured tokens<br/>(provider's own input count,<br/>strongly preferred)"]
    C --> E["estimated tokens<br/>(chars ÷ 4 over the<br/>recorded messages)"]
    D --> F["compact when count ≥ min(window, 120K) − 13K buffer"]
    E --> F
```
...
```mermaid
sequenceDiagram
    participant M as memory.compact()
    participant S as store.replaceMessages()
    participant L as loop.loadHistory()
    M->>M: produce summary (lives in the system prompt)
    M->>S: trim the JSONL log to the preserved turns
    S->>L: reload in-memory history from the trimmed log
```
