...
1. **An ambiguous `edit` silently changes the last occurrence.** For a
   non-`replaceAll` edit, `applyEdit` skips candidates until
   `startIndex === content.lastIndexOf(match)` (`edit.ts:543`) — so an
   `oldString` appearing three times is applied to the *third*, with no error.
   The model has usually just read the top of the file and means the first.
   Claude Code and opencode both hard-fail with "found N matches, add more
   context"; this should too.
2. **`bash` and `agent` declare `isDestructive: false`.** Only `edit` and
   `write` declare `true`, and four things read that flag:
   the verify gate (a run that edits via `sed -i` or `git apply` never
   triggers typecheck), the verifier gate (`mutatedFiles` stays empty), the
   stagnation counter (bash-only work always looks like no progress), and the
   orchestrator's retry rule — whose own comment says "re-running a
   write/edit/bash is not safe" while the data says bash may be re-run. It also
   disagrees with `permission/mode-policy.ts`, which correctly treats both as
   mutating.
3. **Mutating MCP tools are marked concurrency-safe.**
   `convertMcpTool` sets `isConcurrencySafe: true` unconditionally
   (`convert-tool.ts:36`) while deriving `isDestructive` from `readOnlyHint`. So
   a batch of MCP mutations runs in parallel — exactly what the batcher exists
   to prevent, and in the same object that carefully avoids re-running
   `create_issue`.
4. **MCP schema conversion drops `required`, `items`, and nested
   `properties`.** `convertJsonSchema` returns `{ type, properties }` with no
   `required` array, and `convertProperty` keeps only
   `description`/`type`/`enum` (`convert-tool.ts:79`). Required fields are
   therefore never declared to the provider, array element types are lost, and
   `coerceArgs` cannot reach inside an object or array parameter. A schema that
   isn't a plain object-with-properties (`$ref`, `allOf`, `oneOf`) collapses to
   `{ type: "object" }` — the model is told the tool takes no parameters.
5. **`validateParams` is dead** (`orchestrator.ts:64`) — defined, never called.
   Combined with gap 4, a missing required argument on an MCP tool is caught
   nowhere and surfaces as `undefined` inside the remote call.
6. **Four `Tool` fields have no readers at all.** `behavior.maxResultSizeChars`
