...
```mermaid
graph TD
    T["tools/index.ts<br/>tools = { read, write, … } (16, static)<br/>mcpTools = {} (dynamic)"]
    R["registerMcpTool / unregisterMcpTools"] --> T
    T --> G["getTool(id) — built-in first, then MCP"]
    T --> L["listTools() — both, built-in first"]
    T --> D["defs-cache.ts<br/>getToolDefs(): { name, description, parameters }[]<br/>memoized, sorted by name,<br/>invalidated on tools.changed / mcp.tools.changed"]
```
...
```mermaid
graph TD
    A["0. profile check<br/>isToolAllowed(tool, profile) — only if a profile was passed"] --> B["1. lookup<br/>getTool(name) — unknown ⇒ error result"]
    B --> C["2. coerce<br/>coerceArgs(schema, args) — &quot;30&quot; → 30, &quot;false&quot; → false"]
    C --> D["3. validate<br/>tool.validateInput(args)"]
    D --> E["4. permission hook<br/>tool.checkPermissions(args, ctx)"]
    E --> F["5. execute<br/>tool.execute(args, ctx) — retried once if non-destructive"]
    F --> G["6. shape<br/>store full output, truncate the model's copy"]
```
...
Every one of these is derived **after** the full output is stored — see
[Output budgets](#output-budgets) for why that ordering is load-bearing.
...
```mermaid
graph LR
    A["planToolBatches([read, read, grep, edit, read])"] --> B["{0..3, parallel}"]
    A --> C["{3..4, solo}"]
    A --> D["{4..5, solo}"]
```
...
```mermaid
graph LR
    A["full output"] --> B["OutputStore.put(toolCallId, output)<br/>16 MB/session, byte-LRU"]
    A --> C["adaptiveTruncate(output, toolCallId)<br/>30K chars: 24K head + 6K tail"]
    C --> D["marker naming the id, so the model can page the middle"]
```
...
gets decided: [Cost efficiency](/internals/cost-efficiency).
...
[Output budgets](#output-budgets)); the only shaping `bash` itself does is
...
12. **`tool_complete` still crosses IPC in one message.** The event carries
    `result.stdout`, now tail-capped at 500K chars at the orchestrator (it
    used to be fully uncapped), but half a megabyte in a single JSON-RPC
    message is still a lot; a streaming or windowed hand-off has no design
    yet.
