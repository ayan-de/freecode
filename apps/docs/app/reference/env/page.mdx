...
2. **`0` means different things in different rows.** For timeouts it disables the
   guard; for `FREECODE_READ_DEDUP`, `FREECODE_READ_LINE_NUMBERS` and
...
| `FREECODE_READ_LINE_NUMBERS` | on | per call | `0` drops the per-line `N:` prefix from `read` output; the range footer stays. An [experiment](/internals/cost-efficiency): the prefix stays on until an A/B shows the model doesn't rely on it |
