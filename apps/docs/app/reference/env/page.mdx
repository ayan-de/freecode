...
| `FREECODE_NO_UPDATE` | you | `1`, `true`, or `yes` disables the on-launch update check — the way to pin a version, including one run from `builds/versions/<old>/`. An explicit `freecode update` still updates |
