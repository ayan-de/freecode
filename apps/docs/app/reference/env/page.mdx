...
2. **`0` means different things in different rows.** For timeouts it disables the
   guard; for `FREECODE_READ_DEDUP` and `FREECODE_CACHE_MISS_NOTICES` it turns a
   feature off; for `FREECODE_IDLE_NUDGE_TOKENS` it disables the nudge. Nothing
...
| `FREECODE_READ_DEDUP` | on | per call | `0` makes `read` send the full file body every time instead of eliding an unchanged re-read. The only token-efficiency switch that changes what the model *sees* |
...
| --- | --- | --- |
| `FREECODE_BUNDLED` | the release binary (`build:bun`) | `1` means "self-contained binary": core is bundled rather than spawned from disk, the update check runs, and the native-library re-exec happens |
...
`__FREECODE_REEXECED` and `__FREECODE_UPDATE_CHECKED` are internal sentinels the
binary sets on itself to avoid re-exec loops. Do not set them.
