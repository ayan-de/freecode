...
async function connectServer(server: McpServer): Promise<{ name: string; toolCount: number } | null> {
  try {
...
export async function initMcpServers(): Promise<void> {
  ...
  console.error(`[MCP] Initialized ${connected}/${enabledServers.length} servers`);
