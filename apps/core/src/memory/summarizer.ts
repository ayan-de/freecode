 ...
 */
function extractWorkStatus(messages: MemoryMessage[]): { done: string[]; inProgress: string[]; blocked: string[] } {
  const done: string[] = []
  ...
  for (const msg of messages) {
    ...
    if (content.includes("working on") || content.includes("in progress") || content.includes("currently")) {
      const summary = clip(msg.content, 120)
      if (!inProgress.includes(summary)) inProgress.push(summary)
    }
    if (content.includes("blocked") || content.includes("waiting") || content.includes("error")) {
      const summary = clip(msg.content, 120)
...
export function summarizeMessages(input: SummarizeInput): CompactionSummary {
  ...
  // Goal section
  lines.push("## Goal")
  lines.push(`- ${clip(userMessages[0]?.content ?? "Continue the current coding session.", 240)}`)
