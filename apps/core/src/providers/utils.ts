...
export function convertToCoreMessages(messages: Message[]): ModelMessage[] {
  ...
  for (const msg of messages) {
    if (msg.role === 'user') {
      ...
      const textParts: string[] = []
      const toolCalls: Array<{ type: 'tool-call'; toolCallId: string; toolName: string; args: unknown }> = []
      const toolResults: Array<{ type: 'tool-result'; toolCallId: string; toolName: string; output: unknown }> = []
