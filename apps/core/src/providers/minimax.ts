...
function createMiniMaxProvider(_apiKey: string): AIProvider {
  ...
  async function execute(opts: ExecuteOptions): Promise<ExecuteResult> {
    ...
    // MiniMax's `input_tokens` (non-cached only) + `cache_*` breakdown into
    // the inclusive `result.usage.inputTokens` plus `inputTokenDetails`. We
    // just publish the full additive shape and preserve the raw envelope
    // for billing audit. MiniMax hasn't published cache pricing as of writing,
    // but the fields are honored when present.
