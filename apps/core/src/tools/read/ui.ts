...
export function renderReadOutput(
  ...
  const displayLines = options.limit
    ? contentLines.slice(0, options.limit)
    : contentLines;

  displayLines.forEach((line, i) => {
    const lineNum = startLine + i;
    lines.push(
      `${COLORS.dim}${String(lineNum).padStart(4)}${COLORS.reset} ${line}`,
    );
  });
  ...
  if (options.truncated) {
    lines.push(
      `${COLORS.yellow}⚠ Output truncated (use offset to continue)${COLORS.reset}`,
    );
  } else if (options.totalLines !== undefined) {
    lines.push(
      `${COLORS.dim}(End of file - ${options.totalLines} lines)${COLORS.reset}`,
    );
  }
