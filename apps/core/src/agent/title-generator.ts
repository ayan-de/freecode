 ...
 */
export async function generateSessionTitle(opts: GenerateTitleOptions): Promise<string> {
  try {
    ...
    const result = await provider.execute({
      prompt: TITLE_PROMPT_TEMPLATE.replace("{prompt}", opts.prompt.slice(0, 500)),
      system: "You are a helpful assistant that generates concise session titles.",
      model: titleModel,
      maxTokens: 20,
    })
...
function extractSmallModel(model: string): string | undefined {
  ...
  if (model.toLowerCase().includes("haiku") || model.toLowerCase().includes("mini")) {
    return model
  }
